...
import org.ideoholic.fineract.servicecharge.service.ServiceChargeLoanDetailsReadPlatformService;
import org.ideoholic.fineract.servicecharge.service.ServiceChargeLoanDetailsReadPlatformServiceImpl;
import org.ideoholic.fineract.util.ServiceChargeDateUtils.DateIterator;
...
public class ServiceChargeLoanSummaryFactory {
	...
	public ServiceChargeLoanProductSummary getLoanSummaryObject(
			ServiceChargeLoanDetailsReadPlatformService loanReadService, SCLoanAccountData loanAccData,
	...
	public List<BigDecimal> getMonthWiseOutstandingAmount(boolean isDemandLoan) {
		...
		for (Long identifier : loanSummaryObjectMap.keySet()) {
			...
			// The condition in the if is the xnor condition by which the expression becomes
			// true only if both the boolean values are the same
		...
		logger.debug("getMonthWiseOutstandingAmount()::Result :" + result);
		return result;
	}
	...
	private class LoanSummaryQuarterly implements ServiceChargeLoanProductSummary {
		...
		private final Logger logger = LoggerFactory.getLogger(LoanSummaryQuarterly.class);
		...
		BigDecimal populateOutstandingAndRepaymentAmounts(
			...
			Date lastDayOfMonth = quarter.getToDateForCurrentYear();
	...
	private class LoanSummaryDaily implements ServiceChargeLoanProductSummary {
		...
		private final Logger logger = LoggerFactory.getLogger(LoanSummaryDaily.class);
		...
		private void populateOutstandingAndRepaymentAmounts(
				ServiceChargeLoanDetailsReadPlatformService scLoanDetailsReadPlatform, LoanProductData loanProduct,
				SCLoanAccountData loanAccData) {
			// Print the loan details before starting for debugging
			...
			// Start with the last outstanding amount
			BigDecimal loanOutstandingAmount = scLoanDetailsReadPlatform
					.getTotalLoanRepaymentForLoanAfterGivenDate(loanAccData.getId(), lastDayOfRange);
			loanOutstandingAmount = loanOutstandingAmount.add(loanAccData.getPrincipalOutstanding());
			logger.debug("populateOutstandingAndRepaymentAmounts()::loanOutstandingAmount:" + loanOutstandingAmount);
			...
			for (int i = 1; i <= dateRange.getDateRangeDurationMonths(); i++) {
				...
				if (getDisbursmentDate().compareTo(lastDayOfRange) < 0) {
					...
					// Before starting iterator determine the number of times loop has to run
					firstDayOfMonth = ServiceChargeDateUtils.determineSCLoopEndDate(firstDayOfMonth, lastDayOfRange,
							getDisbursmentDate());
					Iterator<Date> dateIterator = new DateIterator(lastDayOfRange, firstDayOfMonth, true);
					logger.debug(
							"populateOutstandingAndRepaymentAmounts(): iteration loop dateIterator:" + dateIterator);
					...
					while (dateIterator.hasNext()) {
						...
						if (repaymentDateAmountMap.containsKey(dateKey)) {
							...
							summationOfDailyOutstanding = addDailyLoanOutstandingSummationOverGivenPeriod(curDate,
									dateMarker, summationOfDailyOutstanding, loanOutstandingAmount);
					...
					summationOfDailyOutstanding = addDailyLoanOutstandingSummationOverGivenPeriod(curDate, dateMarker,
							summationOfDailyOutstanding, loanOutstandingAmount);
			...
			logger.debug("populateOutstandingAndRepaymentAmounts()::outstanding array:" + outstanding);
		}
		...
		private BigDecimal populateRepaymentsIntoMap(Map<String, BigDecimal> repaymentDateAmountMap,
			...
			for (LoanTransactionData loanTransactionData : currentLoanRepayments) {
				...
				if (isRepaymentTransaction(loanTransactionData)) {
					...
					logger.trace(
							"ServiceChargeLoanSummaryFactory.LoanSummaryDaily.populateRepaymentsIntoMap::Map value:"
									+ repaymentAmount + "Map Key:" + dateKey);
		...
		private BigDecimal addDailyLoanOutstandingSummationOverGivenPeriod(Date curDate, Date dateMarker,
				BigDecimal summationOfDailyOutstanding, BigDecimal loanOutstandingAmount) {
			BigDecimal periodBetweenDates = new BigDecimal(
					ServiceChargeDateUtils.getDiffBetweenDates(curDate, dateMarker, 0));
			...
			logger.trace("updateLoanOutstandingAndSummationValues()::curDate:" + curDate + " dateMarker:" + dateMarker
					+ " periodBetweenDates:" + periodBetweenDates + " loanOutstandingAmount:" + loanOutstandingAmount
		...
		private void addPeriodicOutstandingReversed(List<BigDecimal> outstanding) {
			logger.trace("addPeriodicOutstandingReversed::outstanding:" + outstanding);
			for (int iCount = outstanding.size() - 1; iCount >= 0; iCount--) {
				BigDecimal amount = outstanding.get(iCount);
				logger.trace("addPeriodicOutstandingReversed::amount:" + amount.toEngineeringString());
				addPeriodicOutstanding(amount);
			}
		}
