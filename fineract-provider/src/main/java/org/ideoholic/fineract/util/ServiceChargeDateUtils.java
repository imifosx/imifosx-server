...
public class ServiceChargeDateUtils implements ServiceChargeApiConstants {
	...
	/**
	 * Method to check if given two dates are the same. Date check is done for same
	 * date, month and year. This skips the check on the time and only date part of
	 * the given two dates are checked.
	 * 
	 * @param first
	 * @param second
	 * @return
	...
	public static boolean checkIfGivenDatesAreSame(Date first, Date second) {
		...
		if (calFirst.get(Calendar.DATE) != calSecond.get(Calendar.DATE)) {
			return false;
		}
	 ...
	 * 
	 * @param localDate - org.joda.time.LocalDate
	 * @return String
	 */
	...
	/**
	 * Method to check if the given map contains a key with the date that is send as
	 * a key. Make sure that the key of the map is of type Date else there will be
	...
	public static boolean checkIfDateMapContainsGivenDateKey(Map dateMap, Date key) {
		...
		for (Date setKey : mapKeySet) {
			if (checkIfGivenDatesAreSame(key, setKey)) {
				return true;
			}
		}
		return false;
	...
	public static Date determineSCLoopEndDate(Date firstDayOfRange, Date lastDayOfRange, Date disbursmentDate) {
		...
		logger.debug("DateUtils.determineSCLoopEndDate::firstDayOfMonth:" + firstDayOfRange + "disbursmentDate:"
				+ disbursmentDate);
		// In case the loan is disbursed after first day of the range
		if (disbursmentDateShort.after(firstDayOfMonthShort)) {
	...
	public static class DateIterator implements Iterator<Date>, Iterable<Date> {
		...
		public String toString() {
			StringBuffer sb = new StringBuffer();
			sb.append("Start:" + this.current);
			sb.append("End:" + this.end);
			sb.append("Reverse:" + this.isReverse());
			return sb.toString();
		}
