...
import org.ideoholic.fineract.servicecharge.data.ServiceChargeLoanProductSummary;
import org.ideoholic.fineract.util.ServiceChargeLoanSummaryFactory;
...
@Service
public class ServiceChargeLoanDetailsReadPlatformServiceImpl
        implements ServiceChargeLoanDetailsReadPlatformService, ServiceChargeApiConstants {
    ...
    private final SCLoanMapper loaanLoanMapper = new SCLoanMapper();
    private final PaginationHelper<SCLoanAccountData> paginationHelper = new PaginationHelper<>();
    ...
    public BigDecimal getTotalLoansForCurrentQuarter() {
        ...
        Page<SCLoanAccountData> loanAccountData = null;

        loanAccountData = retrieveLoansToBeConsideredForTheQuarter(searchParameters, startDate, endDate);
        ...
        for (int i = 0; i < dataListSize; i++) {
            ...
            try {
                ...
                if (!loanAccountData.getPageItems().get(i).isActive()) {
                    logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.getAllLoansRepaymentData::Loan ID:" + loanId
                            + " is inactive!");
                    continue;
                }
                ...
                for (LoanTransactionData loanTransactionData : currentLoanRepayments) {
                    ...
                    logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.getAllLoansRepaymentData::Partial totalRepayment:"
                            + totalRepayment.toPlainString());
                }

                /*
                 * Get Loan Charge Name - TODO: Need to decide if we need to
                 * identify the loan type String loanCharges =
                 * getLoanChargeName(loanAccountData.getPageItems().get(i).getId
                 * ()); logger.
                ...
                // Exception on a particular loan is ignored and continue to
                // calculate
                e.printStackTrace();
            }
        }
    ...
    public void populateRepaymentsInSheetData(ServiceChargeFinalSheetData sheetData) {
        ...
        if (loanAccountDataForOutstandingAmount != null) {
            for (int i = 0; i < loanAccountDataForOutstandingAmount.getPageItems().size(); i++) {
                SCLoanAccountData loanAccData = loanAccountDataForOutstandingAmount.getPageItems().get(i);
                LoanProductData loanProduct = loanProductReadPlatformService.retrieveLoanProduct(loanAccData.getLoanProductId());
                ...
                logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.populateRepaymentsInSheetData::Account Loan id "
                        + loanAccData.getId());
        ...
        /*
         * This division need not be done in case of daily outstanding
         * calculation but in case of Monthly outstanding then a divsion by
         * fixed number (3) is applicable only on a quarterly based calculation
         */
        // nDLtotalOutstandingAmount = nDLtotalOutstandingAmount.divide(new
        ...
        logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.populateRepaymentsInSheetData::totalOutstanding DL Amount:"
                + dLtotalOutstandingAmount);
        logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.populateRepaymentsInSheetData::totalOutstanding Non DL Amount:"
                + nDLtotalOutstandingAmount);
        logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.populateRepaymentsInSheetData:: DL Monthly cost list:"
                + loanSummaryFactory.getMonthWiseOutstandingAmount(true));
        logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.populateRepaymentsInSheetData:: Non-DL Monthly cost list:"
    ...
    public void getRepaymentsInSheetData(ServiceChargeFinalSheetData sheetData) {
        ...
        // ServiceChargeLoanDetailsReadPlatformServiceImpl.getRepaymentsInSheetData");
        ...
        if (loanAccountDataForOutstandingAmount != null) {
            for (int i = 0; i < loanAccountDataForOutstandingAmount.getPageItems().size(); i++) {
                ...
                // loanAccData.getTotalOutstandingAmount());
                logger.debug(
                        "ServiceChargeLoanDetailsReadPlatformServiceImpl.getRepaymentsInSheetData::Account Loan id " + loanAccData.getId());
                logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.getRepaymentsInSheetData::Outstanding Amount: "
                ...
                if (!isDemandLaon) {
                    ...
                    Date dateDisbursement = loanAccData.getDisbursementDate().toDate();
                    if (dateDisbursement.compareTo(startDate) >= 0 && dateDisbursement.compareTo(endDate) <= 0) {
                        noOfDL++;
                    }
        ...
        sheetData.setLoanOutstandingAmount(dLtotalOutstandingAmount, nDLtotalOutstandingAmount);
        logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.getRepaymentsInSheetData::totalOutstanding DL Amount:"
                + dLtotalOutstandingAmount);
        logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.getRepaymentsInSheetData::totalOutstanding Non DL Amount:"
                + nDLtotalOutstandingAmount);
    }

    private BigDecimal getRepaymentAmount(ServiceChargeFinalSheetData sheetData, SCLoanAccountData loanAccData, Date startDate,
            Date endDate) {
        ...
        int outstandingAmout = loanAccData.getPrincipalOutstanding().compareTo(BigDecimal.ZERO);
        Date dateDisbursement = loanAccData.getDisbursementDate().toDate();
        ...
        for (int j = 0; j < 3; j++) {
            ...
            // Ignore if the date of disbursement is after then current date
            // under consideration
            if (dateDisbursement.compareTo(date) < 0) {
                ...
                if (currentLoanRepayments.isEmpty() && outstandingAmout != 0) {
                    ...
                    for (LoanTransactionData loanTransactionData : currentLoanRepayments) {
                        ...
                        logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.getRepaymentAmount::individual repayment:"
                                + repaymentAmount);
                    }
    ...
    private BigDecimal getTotalOutstandingAmountForGivenQuarter(Long loanId, ServiceChargeDateRange range) {
        ...
        final Collection<LoanTransactionData> currentLoanRepayments = retrieveLoanTransactionsOutstandingPayments(loanId, startDate,
                endDate);

        if (!currentLoanRepayments.isEmpty()) {
            for (LoanTransactionData loanTransactionData : currentLoanRepayments) {
                logger.debug("Date = " + loanTransactionData.dateOf() + "  Repayment Amount = "
            ...
            if (loanAccountData != null) {
                ...
                final Collection<LoanTransactionData> loanRepaymentsPreviousData = retrieveLoanTransactionsOutstandingPaymentsPreviuosData(
                        loanId);
    ...
    public LoanAccountData checkLoanStatus(final SearchParameters searchParameters, Long loanId) {
        try {
            ...
            sqlBuilder
                    .append(" where l.id=? and ( o.hierarchy like ? or transferToOffice.hierarchy like ?) and l.closedon_date IS NOT NULL");
    ...
    public LoanAccountData retrieveOneLoanForCurrentQuarter(final SearchParameters searchParameters, Long loanId, String startDate,
            String endDate) {
        final AppUser currentUser = this.context.authenticatedUser();
        final String hierarchy = currentUser.getOffice().getHierarchy();
        final String hierarchySearchString = hierarchy + "%";
        ...
        try {
            ...
            sqlBuilder.append(" left join m_office transferToOffice on transferToOffice.id = c.transfer_to_office_id ");
            sqlBuilder
                    .append(" where l.id=? and ( o.hierarchy like ? or transferToOffice.hierarchy like ?) and l.disbursedon_date between '"
    ...
    @Override
    public Page<SCLoanAccountData> retrieveLoansToBeConsideredForTheCurrentQuarter() {
        // Get the dates
        ServiceChargeDateRange quarter = ServiceChargeDateRangeFactory.getCurrentDateRange();
        String strStartDate = quarter.getFormattedFromDateString();
        String strEndDate = quarter.getFormattedToDateString();
    ...
    private Page<SCLoanAccountData> retrieveLoansToBeConsideredForTheQuarter(final SearchParameters searchParameters, String startDate,
            String endDate) {
        ...
        // This is to get all the loans that are either active or if inactive
        // then they have to be closed between the dates under question
        ...
        logger.debug("ServiceChargeLoanDetailsReadPlatformServiceImpl.retrieveLoansToBeConsideredForTheQuarter()::SQL being run is: "
                + sqlBuilder.toString());
    ...
    private static final class SCLoanMapper implements RowMapper<SCLoanAccountData> {
        ...
        @Override
        public SCLoanAccountData mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            return SCLoanAccountData.SCLoanDetails(id, loanProductId, principalOutstanding, approvedPrincipal, clientId, 
                    feeChargesDueAtDisbursementCharged,expectedDisbursementDate, actualDisbursementDate);
        }
