...
public class ServiceChargeLoanDetailsReadPlatformServiceImpl
	...
	public ServiceChargeLoanDetailsReadPlatformServiceImpl(LoanReadPlatformService loanReadPlatformService,
			...
			final LoanProductReadPlatformService readPlatformService, final RoutingDataSource dataSource,
	...
	public BigDecimal getTotalLoansForCurrentQuarter() {
		...
		loanAccountData = retrieveLoansToBeConsideredForTheQuarter(startDate, endDate);
	...
	public void populateRepaymentsInSheetData(ServiceChargeFinalSheetData sheetData) {
		...
		/*
		 * This division need not be done in case of daily outstanding calculation but
		 * in case of Monthly outstanding then a divsion by fixed number (3) is
		 * applicable only on a quarterly based calculation
		 */
	...
	private BigDecimal getRepaymentAmount(ServiceChargeFinalSheetData sheetData, SCLoanAccountData loanAccData,
		...
		for (int j = 0; j < 3; j++) {
			...
			if (dateDisbursement.compareTo(date) < 0) {
				...
				final Collection<LoanTransactionData> currentLoanRepayments = retrieveLoanTransactionsMonthlyPayments(
						loanAccData.getId(), new SimpleDateFormat("yyyy-MM-dd").format(date),
	...
	private boolean findIfLoanDisbursedInGivenQuarter(Long loanId, ServiceChargeDateRange range) {
		...
		try {
			LoanAccountData loanAccountData = retrieveOneLoanForCurrentQuarter(searchParameters, loanId, startDate,
					endDate);
			if (loanAccountData == null) {
				result = false;
			} else {
			...
			logger.error(
					"Exception in ServiceChargeLoanDetailsReadPlatformServiceImpl:findIfLoanDisbursedInGivenQuarter::"
							+ e.getMessage());
		}
	...
	private BigDecimal getTotalRepaymentsForGivenQuarter(Long loanId, ServiceChargeDateRange range) {
		...
		final Collection<LoanTransactionData> currentLoanRepayments = retrieveLoanTransactionsMonthlyPayments(loanId,
				startDate, endDate);

		for (LoanTransactionData loanTransactionData : currentLoanRepayments) {
			logger.debug("Date = " + loanTransactionData.dateOf() + "  Repayment Amount = "
					+ loanTransactionData.getAmount());
	...
	private BigDecimal getTotalOutstandingAmountForGivenQuarter(Long loanId, ServiceChargeDateRange range) {
		...
		final Collection<LoanTransactionData> currentLoanRepayments = retrieveLoanTransactionsOutstandingPayments(
		...
		if (!currentLoanRepayments.isEmpty()) {
			for (LoanTransactionData loanTransactionData : currentLoanRepayments) {
				...
				// perform add operation on bg1 with augend bg2 and context mc
				totalOutstandingAmount = totalOutstandingAmount.add(loanTransactionData.getOutstandingLoanBalance(),
						mc);
			}
		} else {
	...
	@Override
	public Collection<LoanTransactionData> retrieveLoanTransactionsOutstandingPayments(Long loanId, String startDate,
			String endDate) {
		try {
	...
	public LoanAccountData checkLoanStatus(final SearchParameters searchParameters, Long loanId) {
		try {
			...
			sqlBuilder.append(
					" where l.id=? and ( o.hierarchy like ? or transferToOffice.hierarchy like ?) and l.closedon_date IS NOT NULL");
	...
	public LoanAccountData retrieveOneLoanForCurrentQuarter(final SearchParameters searchParameters, Long loanId,
		...
		try {
			...
			sqlBuilder.append(" left join m_office transferToOffice on transferToOffice.id = c.transfer_to_office_id ");
			sqlBuilder.append(
					" where l.id=? and ( o.hierarchy like ? or transferToOffice.hierarchy like ?) and l.disbursedon_date between '"
	...
	public Page<SCLoanAccountData> retrieveLoansToBeConsideredForTheCurrentQuarter() {
		...
		return retrieveLoansToBeConsideredForTheQuarter(strStartDate, strEndDate);

	}

	private Page<SCLoanAccountData> retrieveLoansToBeConsideredForTheQuarter(String startDate, String endDate) {

		final SCLoanMapper loaanLoanMapper = new SCLoanMapper();
		final AppUser currentUser = this.context.authenticatedUser();
		final String hierarchy = currentUser.getOffice().getHierarchy();
		final String hierarchySearchString = hierarchy + "%";
		...
		sqlBuilder.append(" LEFT JOIN m_office transferToOffice ON transferToOffice.id = c.transfer_to_office_id ");
		sqlBuilder.append(" LEFT JOIN m_office goffice ON goffice.id = g.office_id");
		sqlBuilder.append(" WHERE ( o.hierarchy LIKE ? OR transferToOffice.hierarchy LIKE ? ");
		sqlBuilder.append(" OR goffice.hierarchy LIKE ?)");
		// This is to add conditions to get loans that are/were active during period
		sqlBuilder.append(" AND l.rejectedon_date IS NULL"); // Filter out rejected loans
		// Ensure disbursal had occurred and before the end date of given period
		sqlBuilder.append(" AND (l.disbursedon_date IS NOT NULL AND l.disbursedon_date <= ? )");
	...
	private static final class SCLoanMapper implements RowMapper<SCLoanAccountData> {
		...
		public String loanSchema() {
			return "l.id as id, l.account_no as accountNo, l.external_id as externalId, l.fund_id as fundId, "
					...
					+ " l.submittedon_date as submittedOnDate, " + " l.rejectedon_date as rejectedOnDate, "
					+ " l.withdrawnon_date as withdrawnOnDate, " + " l.approvedon_date as approvedOnDate, "
					+ " l.expected_disbursedon_date as expectedDisbursementDate, l.disbursedon_date as actualDisbursementDate, "
					+ " l.closedon_date as closedOnDate, l.writtenoffon_date as writtenOffOnDate, "
					...
					+ " l.loan_officer_id as loanOfficerId," + " l.principal_disbursed_derived as principalDisbursed,"
	...
	private static final class LoanMapper implements RowMapper<LoanAccountData> {
		...
		public String loanSchema() {
			return "l.id as id, l.account_no as accountNo, l.external_id as externalId, l.fund_id as fundId, f.name as fundName,"
					...
					+ " lp.can_use_for_topup as canUseForTopup, " + " l.is_topup as isTopup, "
					+ " topup.closure_loan_id as closureLoanId, " + " topuploan.account_no as closureLoanAccountNo, "
		...
		public LoanAccountData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
			...
			return LoanAccountData.basicLoanDetails(id, accountNo, status, externalId, clientId, clientAccountNo,
					...
					topupAmount, isEqualAmortization);
		}
	}
}
