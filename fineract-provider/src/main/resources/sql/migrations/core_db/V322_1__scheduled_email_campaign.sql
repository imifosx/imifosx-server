...
DELETE FROM `job` WHERE `name`= 'Execute Email' AND `display_name`='Execute Email';
insert into job (name, display_name, cron_expression, create_time)
values ('Execute Email', 'Execute Email', '0 0/10 * * * ?', NOW());

DELETE FROM `job` WHERE `name`= 'Update Email Outbound with campaign message' AND `display_name`='Update Email Outbound with campaign message';
insert into job (name, display_name, cron_expression, create_time)
values ('Update Email Outbound with campaign message', 'Update Email Outbound with campaign message', '0 0/15 * * * ?', NOW());

DELETE FROM `scheduled_email_configuration` WHERE `name` IN ('SMTP_SERVER', 'SMTP_PORT', 'SMTP_USERNAME', 'SMTP_PASSWORD');
